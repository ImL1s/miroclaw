...
class SocialEnvironment(Environment):
    ...
    async def get_followers_env(self) -> str:
        ...
        if hasattr(self.action.channel, "get_context"):
            ctx = await self.action.channel.get_context(agent_id)
            num_followers = ctx.get("num_followers", 0)
        else:
            db_path = get_db_path()
            try:
    ...
    async def get_follows_env(self) -> str:
        ...
        if hasattr(self.action.channel, "get_context"):
            ctx = await self.action.channel.get_context(agent_id)
            num_followings = ctx.get("num_followings", 0)
        else:
            try:
                db_path = get_db_path()
                conn = sqlite3.connect(db_path)
